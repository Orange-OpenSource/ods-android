...
import com.orange.ods.demo.ui.components.menus.ComponentMenuContentState.Companion.MenuItemCount
...
fun ComponentMenu(variant: Variant) {
    val customizationState = rememberMenuCustomizationState()
    ...
    with(customizationState) {
        ComponentCustomizationBottomSheetScaffold(
            ...
            Column(
                ...
                if (variant == Variant.DropdownMenu) {
                    OdsTextBody1(
                        modifier = Modifier
                    ...
                    Box(modifier = Modifier.padding(top = dimensionResource(id = R.dimen.spacing_s))) {
                        ...
                        OdsDropdownMenu(expanded = menuExpanded, onDismissRequest = { menuExpanded = false }, offset = DpOffset(x = (-100).dp, y = (-10).dp)) {
                            LocalRecipes.current.take(MenuItemCount).forEachIndexed { index, recipe ->
                                OdsDropdownMenuItem(
                                    text = recipe.title,
