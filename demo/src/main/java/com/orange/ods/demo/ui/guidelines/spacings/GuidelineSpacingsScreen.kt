...
fun GuidelineSpacingsScreen(updateTopBarTitle: (Int) -> Unit) {
    ...
    LazyColumn(contentPadding = PaddingValues(bottom = dimensionResource(id = R.dimen.spacing_m))) {
        item {
            ComponentHeader(imageRes = R.drawable.il_spacings, description = R.string.guideline_spacing_description)
        }
        item {
            OdsTextSubtitle1(
                modifier = Modifier
        ...
        items(Spacing.values()) { spacing ->
            ...
            OdsListItem(
                ...
                secondaryText = stringResource(id = R.string.guideline_spacing_dp, spacing.dp.value.toInt()) + "\n",
                singleLineSecondaryText = false,
                icon = { GuidelineSpacingImage(spacing = spacing) },
                trailing = {
                    Text(
                        text = stringResource(id = R.string.guideline_spacing_ratio, if (spacing.ratio == 0.0f) "-" else ratioFormatter.format(spacing.ratio)),
