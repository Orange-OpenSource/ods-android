...
import com.orange.ods.compose.component.button.OdsIconToggleButtonsRowIconBuilder
...
fun ButtonsIconToggleGroup(customizationState: ButtonIconCustomizationState) {
    val icons =
        LocalRecipes.current.distinctBy { it.iconResId }.filter { it.iconResId != null }.take(ButtonIconCustomizationState.MaxToggleCount).map { recipe ->
            OdsIconToggleButtonsRowIconBuilder(painterResource(id = recipe.iconResId!!), recipe.title, customizationState.enabled.value)
        }
                    ...
                    parameters = {
                        list("icons") {
                            repeat(toggleCount.value) {
                                classInstance<OdsIconToggleButtonsRowIconBuilder> {
                                    painter()
                                    contentDescription("")
